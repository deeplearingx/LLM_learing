...
class VLMConfig(PretrainedConfig):
    ...
    def __init__(self, llm_model_path='/root/autodl-tmp/ai_learing/models/Qwen/Qwen2.5-0.5B-Instruct',
                 vision_model_path='/root/autodl-tmp/ai_learing/models/google/siglip2-base-patch16-224',
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            pixel_values = self.processor(images=default_image, return_tensors="pt")['pixel_values'].squeeze(0)
            q_text = self.tokenizer.apply_chat_template([{"role": "system", "content": 'You are a helpful assistant.'},
...
class MyDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            pixel_values.append(feature['pixel_values'])  # 这里已经是 Tensor 了

        return {
            'input_ids': torch.tensor(input_ids, dtype=torch.long),
            'labels': torch.tensor(labels, dtype=torch.long),
            'pixel_values': torch.stack(pixel_values, dim=0),  # ✅ 改成 stack
        }


if __name__ == '__main__':
    config = VLMConfig(vision_model_path='/root/autodl-tmp/ai_learing/models/google/siglip2-base-patch16-224',
                       image_pad_num=49)
    model = VLM(config).cuda()
    print(model)
    print(f'模型参数量为：{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    images_path = '/root/autodl-tmp/ai_learing/datasets/LLaVA-CC3M-Pretrain-595K/images'
    data_path = '/root/autodl-tmp/ai_learing/datasets/ Chinese-LLaVA-Vision-Instructions/chat-translated.json'
